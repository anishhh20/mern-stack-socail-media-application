...
const LinkedInWidget = ({ userId, linkedInLink }) => {
  ...
  const isValid = isLinkedInLinkValid(link);
    ...
    if (isValid || link === "") {
      try {
        const response = await fetch(
          `http://localhost:3001/users/${userId}/saveLinkedInLink`,
          {
            method: "PUT",
            headers: {
            ...
            body: JSON.stringify({ userId: userId, linkedInLink: link }),
          }
        );
        ...
        if (response.ok) {
          ...
          console.error("Error updating LinkedIn link:", response.statusText);
        }
      } catch (error) {
