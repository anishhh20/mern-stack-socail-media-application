...
export const getSuggestions = async (req, res) => {
  try {
    ...
    const suggestions = await User.find({
      _id: {
        $nin: [id, ...user.friends], // Exclude user, friends
      },
    });

    const getRandomUsers = (suggestions, numberOfSuggestions) => {
      const shuffledUsers = suggestions.sort(() => 0.5 - Math.random());
      return shuffledUsers.slice(0, numberOfSuggestions);
    };

    res.json(getRandomUsers(suggestions, 6));
    await user.save();
  } catch (err) {
    res.status(404).json({ message: err.message });
  }
